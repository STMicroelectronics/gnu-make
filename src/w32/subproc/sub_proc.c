...
static wchar_t *make_command_line(char *shell_name, char *exec_path, char **argv);
static wchar_t *make_command_line_1(wchar_t *shell_name, wchar_t *exec_path, wchar_t **argv);
        ...
        char *as_user)
{
        ...
        if (as_user) {
                ...
                wchar_t *wexec_path = handle_long_path_utf8(exec_path);
                if (!wexec_path) {
                        free(envblk);
                        free( command_line );
                        return -1;
                ...
                DB (DB_JOBS, ("CreateProcessW(%ls,%ls,...)\n",
                        *wexec_path ? wexec_path : L"NULL",
                ...
                if (CreateProcessW(
                        ...
                        pproc->last_err = GetLastError();
                        pproc->lerrno = E_FORK;
                        fprintf(stderr, "process_begin: CreateProcessW(%ls, %ls, ...) failed.\n",
                                wexec_path ? wexec_path : L"NULL", command_line);
                        free(envblk);
                        free( command_line );
                        free(wexec_path);
                        return(-1);
...
{
  wchar_t *wshell_name = handle_long_path_utf8(shell_name);
  wchar_t *wfull_exec_path = handle_long_path_utf8(full_exec_path);

  if ((shell_name && !wshell_name) || (full_exec_path && !wfull_exec_path)) {
    free(wshell_name);
    free(wfull_exec_path);
    return NULL;
  }
  ...
  wargv[argc] = NULL; /* NULL terminate the list.  */

  wchar_t *command_line = make_command_line_1(wshell_name, wfull_exec_path, wargv);
...
static wchar_t *
make_command_line_1( wchar_t *shell_name, wchar_t *full_exec_path, wchar_t **argv)
{
        int    argc = 0;
        wchar_t** argvi;
        ...
        if (shell_name && full_exec_path) {
                have_sh = cygwin_mode && wcsstr(full_exec_path, L"sh.exe");
                bytes_required
                  = wcslen(shell_name) + 1 + wcslen(full_exec_path);
                /*
                 ...
                 */
                if (*argv && wcscmp(full_exec_path, L"-c")) argv++;
                /*
        ...
        command_line = (wchar_t*) malloc(bytes_required * sizeof(wchar_t));
